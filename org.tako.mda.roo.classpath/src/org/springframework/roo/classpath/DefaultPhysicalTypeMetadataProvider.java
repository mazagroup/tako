 ...
 * @since 1.2.0
 */
@Component( service = {PhysicalTypeMetadataProvider.class,MetadataProvider.class,FileEventListener.class} )
public class DefaultPhysicalTypeMetadataProvider implements PhysicalTypeMetadataProvider,
    FileEventListener {

  protected final static Logger LOGGER = HandlerUtils
      .getLogger(DefaultPhysicalTypeMetadataProvider.class);
  ...
  @Reference(name = "memberHoldingDecorator",policy = ReferencePolicy.DYNAMIC,service=MemberDetailsDecorator.class,cardinality = ReferenceCardinality.MULTIPLE)
  ...
  public MetadataItem get(final String metadataIdentificationString) {
    ...
    Validate.isTrue(PhysicalTypeIdentifier.isValid(metadataIdentificationString),
        "Metadata id '%s' is not valid for this metadata provider", metadataIdentificationString);
    final String canonicalPath =
        typeLocationService.getPhysicalTypeCanonicalPath(metadataIdentificationString);
    ...
    final ClassOrInterfaceTypeDetails typeDetails =
        typeParsingService.getTypeAtLocation(canonicalPath, metadataIdentificationString, javaType);
    if (typeDetails == null) {
      return null;
    }
    final PhysicalTypeMetadata result =
        new DefaultPhysicalTypeMetadata(metadataIdentificationString, canonicalPath, typeDetails);
    final ClassOrInterfaceTypeDetails details = result.getMemberHoldingTypeDetails();
    if (details != null && details.getPhysicalTypeCategory() == PhysicalTypeCategory.CLASS
      ...
      if (details.getSuperclass() != null) {
        ...
        for (final LogicalPath sourcePath : projectOperations.getPathResolver().getSourcePaths()) {
          final String possibleSuperclass =
              PhysicalTypeIdentifier.createIdentifier(details.getExtendsTypes().get(0), sourcePath);
    ...
    while (additionalLoopRequired) {
      ...
      for (final MemberDetailsDecorator decorator : decorators) {
        final MemberDetails newResult =
            decorator.decorateTypes(DefaultPhysicalTypeMetadataProvider.class.getName(),
                memberDetails);
        Validate.isTrue(newResult != null, "Decorator '%s' returned an illegal result", decorator
            .getClass().getName());
    ...
    return new DefaultPhysicalTypeMetadata(metadataIdentificationString, canonicalPath,
        (ClassOrInterfaceTypeDetails) memberDetails.getDetails().get(0));
  }
  ...
  public MetadataDependencyRegistry getMetadataDependencyRegistry() {
    ...
    try {
      ...
      LOGGER
          .warning("Cannot load MetadataDependencyRegistry on DefaultPhysicalTypeMetadataProvider.");
      return null;
  ...
  public TypeParsingService getTypeParsingService() {
    ...
    try {
      ServiceReference<?>[] references =
          this.context.getAllServiceReferences(TypeParsingService.class.getName(), null);
