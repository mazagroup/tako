...
public class Execution implements Comparable<Execution> {
   ...
   * @since 1.2.0
   */
  public Execution(final String id, final String phase, final Configuration configuration,
      final String... goals) {
    Validate.notNull(id, "execution id must be specified");
    Validate.notNull(phase, "execution phase must be specified");
    this.configuration = configuration;
    this.goals =
        goals == null ? Collections.<String>emptyList() : Collections.unmodifiableList(Arrays
            .asList(goals));
    this.id = id.trim();
