...
import org.bndtools.templating.jgit.GitCloneTemplate;
 ...
 * @since 1.0
 */
@Component(service = { BndOperations.class, ProjectOperations.class })
public class BndOperationsImpl extends AbstractProjectOperations implements BndOperations {

	protected static final Logger LOGGER = HandlerUtils.getLogger(BndOperationsImpl.class);
	...
	private TemplateLoader workspaceTemplateLoader;
	...
	protected void bindFileManager(final FileManager fileManager) {
		super.fileManager = fileManager;
	}

	protected void unbindFileManager(final FileManager fileManager) {
		super.fileManager = fileManager;
	}

	@Reference(policy = ReferencePolicy.DYNAMIC)
	protected void bindMetadataService(final MetadataService metadataService) {
		super.metadataService = metadataService;
	}

	protected void unbindMetadataService(final MetadataService metadataService) {
		super.metadataService = metadataService;
	}

	@Reference(policy = ReferencePolicy.DYNAMIC)
	protected void bindPathResolver(final PathResolver pathResolver) {
		super.pathResolver = pathResolver;
	}

	protected void unbindPathResolver(final PathResolver pathResolver) {
		super.pathResolver = pathResolver;
	}

	@Reference(policy = ReferencePolicy.DYNAMIC)
	protected void bindShell(final Shell shell) {
		super.shell = shell;
	}

	protected void unbindShell(final Shell shell) {
		super.shell = null;
	}


	@Reference(policy = ReferencePolicy.DYNAMIC)
	protected void bindPomManagementService(final PomManagementService pomManagementService) {
		super.pomManagementService = pomManagementService;
	}

	protected void unbindPomManagementService(final PomManagementService pomManagementService) {
		super.pomManagementService = null;
	}

	@Reference(name = "feature", policy = ReferencePolicy.DYNAMIC, service = Feature.class, cardinality = ReferenceCardinality.MULTIPLE)
	protected void bindFeature(final Feature feature) {
		super.bindFeature(feature);
	}

	protected void unbindFeature(final Feature feature) {
		super.unbindFeature(feature);
	}
	...
	public void createModule(final String moduleName, final PackagingProvider selectedPackagingProvider,
			final String artifactId) {
		createModule(moduleName, selectedPackagingProvider, artifactId, null);
	}

	private void createModule(final String moduleName, final PackagingProvider selectedPackagingProvider,
			final String artifactId, final String folder) {
		...
		final String pathToNewPom = packagingProvider.createArtifacts(
				...
				new GAV(parentPom.getGroupId(), parentPom.getArtifactId(), parentPom.getVersion()), moduleName, this);
	...
	public void createMultimoduleProject(final JavaPackage topLevelPackage, final String projectName,
		...
		final PackagingProvider parentPackagingProvider = getPackagingProvider(
				getPackagingProviderRegistry().getPackagingProvider("pom"));
		final PackagingProvider warPackagingProvider = getPackagingProvider(
				getPackagingProviderRegistry().getPackagingProvider("war"));
		final PackagingProvider jarPackagingProvider = getPackagingProvider(
				getPackagingProviderRegistry().getPackagingProvider("jar"));

		// Create parent pom
		parentPackagingProvider.createArtifacts(topLevelPackage, projectName, getJavaProductVersion(majorJavaVersion),
				null, "", this);
	...
	private void installApplicationConfiguration(String moduleName) {
		...
		final List<Element> dependencies = XmlUtils.findElements("/configuration/dependencies/dependency",
				configuration);
	...
	public void createProject(final JavaPackage topLevelPackage, final String projectName,
		...
		packagingProvider.createArtifacts(topLevelPackage, projectName, getJavaProductVersion(majorJavaVersion), null,
				"", this);
	...
	/**
	 * This method creates a banner.txt file inside generated project that will be
	 * displayed when the generated Spring Boot application starts.
	 *
	 * @param Pom
	...
	/**
	 * Creates topLevelPackage folder structure inside the focused module. If folder
	 * is not null, adds this new folder inside topLevelPackage folders
	 *
	 * @param topLevelPackage
	...
	public void createSpringBootApplicationClass(JavaPackage topLevelPackage, String projectName) {
		...
		final JavaType javaType = new JavaType(
				topLevelPackage.getFullyQualifiedPackageName().concat(".").concat(bootClass));
		final String physicalPath = getPathResolver().getFocusedCanonicalPath(Path.SRC_MAIN_JAVA, javaType);
	 ...
	 *            the major version provided by the user; can be <code>null</code>
	 *            to auto-detect it
	...
	private void updateParentModulePom(final String moduleName) {
		...
		final String addModuleMessage = getDescriptionOfChange(ADDED, Collections.singleton(moduleName), "module",
				"modules");
	...
	public PackagingProviderRegistry getPackagingProviderRegistry() {
		...
		try {
			ServiceReference<?>[] references = this.context
					.getAllServiceReferences(PackagingProviderRegistry.class.getName(), null);
	...
	public ProcessManager getProcessManager() {
		...
		try {
			ServiceReference<?>[] references = this.context.getAllServiceReferences(ProcessManager.class.getName(),
					null);
	...
	@Override
	public void createWorkspaceFromGitTemplate(String workspaceName, String templateType, Integer majorJavaVersion) {
		...
		try {
			...
			LOGGER.warning("Unable to locate at least one TemplateLoader");
		}

        // Find template
		Template targetTemplate = null;
        try {
			targetTemplate = findTemplateByType(promises,templateType);
		} catch (Exception e) {
			LOGGER.severe("Unable to find template by type: "+templateType);
		}

        // Extract template contents
        ResourceMap templateOutputs = null;
        try {
			if (targetTemplate != null) {
				templateOutputs = targetTemplate.generateOutputs(Collections.<String, List<Object>> emptyMap());
			}
		} catch (Exception e) {
			LOGGER.severe("Unable to find template contents in: "+targetTemplate.getName());
		}


        //Write to disk
        for (Entry<String, org.bndtools.templating.Resource> entry : templateOutputs.entries()) {
            // Check for existing files
           	String fileIdentifier = projectOperations.getPathResolver().getIdentifier(LogicalPath.getInstance(Path.ROOT, ""), entry.getKey());
            switch (entry.getValue()
                .getType()) {
                case Folder :
                	if (getFileManager().exists(fileIdentifier)) {
                		LOGGER.severe(String.format("Path already exists and is not a directory: %s", entry.getKey()));
                	}
                	else {
                		getFileManager().createDirectory(fileIdentifier);
                	}
                    break;
                case File :
                	if (getFileManager().exists(fileIdentifier)) {
                		LOGGER.severe(String.format("Path already exists and is not a plain file: %s", entry.getKey()));
                	}
                	else {
                		getFileManager().createOrUpdateTextFileIfRequired(fileIdentifier, entry.getValue().toString(), true);
                	}
                    break;
                default :
                    // ignore
	...
	private Template findTemplateByType(List<Pair<String, Promise<? extends Collection<Template>>>> promises, String templateType) {
        for (Pair<String, Promise<? extends Collection<Template>>> namedPromise : promises) {
            ...
            try {
                Throwable failure = namedPromise.getSecond()
                    .getFailure();
                if (failure != null)
                    LOGGER.log(Level.SEVERE,"Failed to load from template loader: " + name);
                else {
                    Collection<Template> loadedTemplates = namedPromise.getSecond().getValue();
                    for (Template template : loadedTemplates) {
                    	if (template instanceof GitCloneTemplate) {
                    		if (((GitCloneTemplate)template).getName().equals(templateType)) {
                    			return template;
                    		}
                    	}
                    }
            	...
            	LOGGER.log(Level.SEVERE,"Interrupted while loading from template loader: " + name,e);
            } catch (InvocationTargetException e) {
            	LOGGER.log(Level.SEVERE,"InvocationTargetException while loading from template loader: " + name,e);
            }
        }
        return null;
	}
	...
	@Override
	public void createProjectFromGitTemplate(JavaPackage topLevelPackage, String projectName) {
		// TODO Auto-generated method stub
	...
	@Override
	public boolean isCreateWorkspaceAvailable() {
		// TODO Auto-generated method stub
		return false;
	}
}
